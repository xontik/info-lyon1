<main class="container">
                        ...
                        <ul class="collection with-header <?= $data['editable'] ? 'connectedSortable' : ''  ?>"
                            ...
                            if (count($group->students)) {
                                foreach ($group->students as $student) { ?>
                                    <li class="collection-item" data-group-id="<?= $group->idGroup ?>"
                                    ...
                                    if (isset($data['educations'][$group->idGroup][$subject->idSubject])) {
                                        ...
                                        <td>
                                            <i class="small material-icons tooltipped"
                                               ...
                                               data-tooltip="<?php echo $education->name . ' ' . $education->surname; ?>"
                                               data-delay="0">person</i>
                                        </td>
                                        <?php
                                    } else { ?>
                                        <td>
    ...
    if ($data['editable']) { ?>
        <div id="assoctiationCard" class="card grey lighten-5">
            <form action="<?= base_url('Process_Education/add_teacher/'.$data['semester']->idSemester) ?>" method="post">
                ...
                <div class="card-action">
                    <button class="btn-flat waves-effect waves-light" type="submit">Attribuer</button>
                </div>
            </form>
        </div>
        <?php
